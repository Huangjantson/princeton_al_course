...
public class FastCollinearPoints {
	...
	public FastCollinearPoints(Point[] points) {
		...
		for ( Point A : this.points )
			if (A==null) throw new NullPointerException("Containing point with value as null.");
	...
	public LineSegment[] resizeSegemntArray(LineSegment[] segmentArray,int size){
		LineSegment[] temp;
	...
	public LineSegment[] segments(){
		...
		boolean metSmaller;//record if the contiunous point on the slope is smaller than poins[i]
		//iterator over all the points;at least four point in a segment so no need to iterate over all
		...
		{	
			...
			for(int j=0;j<total-1;j++)
			{
				...
				else
				{
					if (continuous>0)
					{
						if((continuous>1)&&(metSmaller==false))
